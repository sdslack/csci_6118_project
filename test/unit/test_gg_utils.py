...
class test_query_file_to_filter(unittest.TestCase):
    ...
    @classmethod
    def setUp(self):
        data = {'Filter Criteria': ['=IRAN'], 'Search_Options':['Country'],
        ...
        data2 = {'Filter Criteria': ["=IRAN; !=FRANCE"], 'Search_Options':['Country'],
               'Search by this column?':['Yes']}
        self.query_format2 = pd.DataFrame(data2)
        self.query_format2.to_csv('../data/two_query_unittest.csv', index=False)
        
        data_wrongcol = {'Filter Criteria': ["=IRAN; !=FRANCE"], 'Wrong_Name':['Country'],
               'Search by this column?':['Yes']}
        self.query_format_wrongcol = pd.DataFrame(data_wrongcol)
        self.query_format_wrongcol.to_csv('../data/wrongcol_query_unittest.csv', index=False)

        data3 = {'Filter Criteria': ["=IRAN; !=FRANCE", "=0"], 
                         'Search_Options':['Country', 'Days of Infection'],
                         'Search by this column?':['Yes', 'Yes']}
        self.query_format3 = pd.DataFrame(data3)
        self.query_format3.to_csv('../data/three_query_unittest.csv', index=False)
    ...
    def test_wrong_colnames(self):
        with self.assertRaises(ValueError):
            pf.query_file_to_filter('../data/wrongcol_query_unittest.csv')
    
    def test_file_empty(self):
        self.assertEqual(len(pf.query_file_to_filter('../data/empty_query.csv')), 0)
        
    def test_file_1filter(self):
        self.assertEqual(len(pf.query_file_to_filter('../data/one_query_unittest.csv')), 1)

    def test_file_2filter(self):
        self.assertEqual(len(pf.query_file_to_filter('../data/two_query_unittest.csv')), 1)

    def test_file_3filter(self):
        self.assertEqual(len(pf.query_file_to_filter('../data/three_query_unittest.csv')), 2)
...
class test_subset_dataframe_by_names(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data = pd.DataFrame({'Patient': [0, 1, 2, 3],
                                  'Country': ['IRAN', 'IRAN', 'FRANCE', 'EGYPT']})
...
class test_consort_filter_data(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.filters = {'Column_1':['>3'], 'Column_3': ['!=4', '<=9', '>=10']}
        ...
        data_strint = {'Column_1': [1, 2, 4, 2, 1],
                    'Column_2': ["Red", "Blue", "Blue", "Green", "Purple"]}
        ...
        self.data_long_filtered_strint = pf.consort_filter_data(self.df_long_strint,
                                              self.filters_strint)
        # Create a file
        pf.consort_filter_data(self.df_long,
                               self.filters, "../data/test_consort_filter_bool_csv.csv")
    ...
    def test_consort_filtered_floats(self):
        ...
        self.assertTrue(all(self.data_long_filtered['Filtered_Column_Column_1'] \
    ...
    def test_consort_filtered_string_int(self):
        ...
        self.assertTrue(all(self.data_long_filtered_strint['Filtered_Column_Column_1'] \
    ...
    def test_write_boolean_csv(self):
        self.assertTrue(os.path.exists("../data/test_consort_filter_bool_csv.csv")) 
    ...
    def test_wrong_write_boolean_csv(self):
        with self.assertRaises(SystemExit):
            pf.consort_filter_data(self.df_long,
                                   self.filters,
                                   "../data/wrongoutput/nopath/outbool_csv.csv")
...
class test_make_query_df_formatted(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.emptyfilters = ""
        self.consort_input_data = pd.read_csv('../data/LANL_HIV1_2023_seq_metadata.csv')
    ...
    def test_nofilter_nosummary(self):
        with self.assertRaises(ValueError):
            pf.make_query_df_formatted(filters = self.filters,
                                       query_summary_file \
                                       = "../data/example_request_summary.csv",
                                       consort_input_data= self.consort_input_data)
            
    def test_nofilter_wrongsummary(self):
        with self.assertRaises(SystemExit):
            pf.make_query_df_formatted(query_summary_file \
                                       = "../data/doesntexist_request_summary.csv")
...
class test_run_consort_plot_rcode(unittest.TestCase):
    ...
    def setUp(self):
        self.query_empty = pd.DataFrame(columns = ['Search_Options',
                                                  'Filter Criteria',
                                                 "Search by this column?"])
        self.data = pd.DataFrame({'Country': ['IRAN', 'IRAN', 'FRANCE', 'EGYPT'],
                                   'Filtered_Column_Country': [True, True, False, False]})

        query_r = {'Filter Criteria': ["=IRAN"], 'Search_Options':['Country']}
        self.query_format_r = pd.DataFrame(query_r)
        # Generate a png file
        pf.run_consort_plot_rcode(self.data, self.query_format_r, "../data/output.png")
